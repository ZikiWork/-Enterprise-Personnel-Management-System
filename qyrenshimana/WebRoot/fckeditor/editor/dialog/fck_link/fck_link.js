...
oParser.Parse兼职信息Params = function(sParams)
{
	// Initialize the o兼职信息Params object.
	var o兼职信息Params = new Object() ;
	o兼职信息Params.Subject = '' ;
	o兼职信息Params.Body = '' ;

	var aMatch = sParams.match( /(^|^\?|&)subject=([^&]+)/i ) ;
	if ( aMatch ) o兼职信息Params.Subject = decodeURIComponent( aMatch[2] ) ;

	aMatch = sParams.match( /(^|^\?|&)body=([^&]+)/i ) ;
	if ( aMatch ) o兼职信息Params.Body = decodeURIComponent( aMatch[2] ) ;

	return o兼职信息Params ;
}

// This method returns either an object containing the 兼职信息 info, or FALSE
// if the parameter is not an 兼职信息 link.
oParser.Parse兼职信息Uri = function( sUrl )
{
	// Initializes the 兼职信息Info object.
	var o兼职信息Info = new Object() ;
	o兼职信息Info.Address = '' ;
	o兼职信息Info.Subject = '' ;
	o兼职信息Info.Body = '' ;
	...
	{
		// This seems to be an unprotected 兼职信息 link.
		var aParts = aLinkInfo[2].match( /^([^\?]+)\??(.+)?/ ) ;
		if ( aParts )
		{
			// Set the e-mail address.
			o兼职信息Info.Address = aParts[1] ;

			// Look for the optional e-mail parameters.
			if ( aParts[2] )
			{
				var o兼职信息Params = oParser.Parse兼职信息Params( aParts[2] ) ;
				o兼职信息Info.Subject = o兼职信息Params.Subject ;
				o兼职信息Info.Body = o兼职信息Params.Body ;
			}
		}
		return o兼职信息Info ;
	}
	...
	{
		// This may be a protected 兼职信息.

		// Try to match the url against the 兼职信息ProtectionFunction.
		var func = FCKConfig.兼职信息ProtectionFunction ;
		if ( func != null )
		{
			try
			{
				...
				if ( aMatch )
				{
					...
					// Fill the 兼职信息Info object that will be returned
					o兼职信息Info.Address = aInfo['NAME'] + '@' + aInfo['DOMAIN'] ;
					o兼职信息Info.Subject = decodeURIComponent( aInfo['SUBJECT'] ) ;
					o兼职信息Info.Body = decodeURIComponent( aInfo['BODY'] ) ;
		...
		// Try to match the 兼职信息 against the encode protection.
		var aMatch = aLinkInfo[2].match( /^location\.href='mailto:'\+(String\.fromCharCode\([\d,]+\))\+'(.*)'$/ ) ;
		if ( aMatch )
		{
			// The link is encoded
			o兼职信息Info.Address = eval( aMatch[1] ) ;
			if ( aMatch[2] )
			{
				var o兼职信息Params = oParser.Parse兼职信息Params( aMatch[2] ) ;
				o兼职信息Info.Subject = o兼职信息Params.Subject ;
				o兼职信息Info.Body = o兼职信息Params.Body ;
			}
			return o兼职信息Info ;
...
oParser.Create兼职信息Uri = function( address, subject, body )
{
	// Switch for the 兼职信息Protection setting.
	switch ( FCKConfig.兼职信息Protection )
	{
		case 'function' :
			var func = FCKConfig.兼职信息ProtectionFunction ;
			if ( func == null )
			{
				if ( FCKConfig.Debug )
				{
					alert('兼职信息Protection alert!\nNo function defined. Please set "FCKConfig.兼职信息ProtectionFunction"') ;
				}
				return '';
			}

			// Split the 兼职信息 address into name and domain parts.
...
function LoadSelection()
{
	...
	// Search for a protected 兼职信息 link.
	var o兼职信息Info = oParser.Parse兼职信息Uri( sHRef );

	if ( o兼职信息Info )
	{
		sType = '兼职信息' ;

		GetE('txt兼职信息Address').value = o兼职信息Info.Address ;
		GetE('txt兼职信息Subject').value = o兼职信息Info.Subject ;
		GetE('txt兼职信息Body').value    = o兼职信息Info.Body ;
	}
	else if ( sProtocol )
	{
...
function SetLinkType( linkType )
{
	...
	ShowE('divLinkType兼职信息'	, (linkType == '兼职信息') ) ;
...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
		...
		case '兼职信息' :
			sUri = GetE('txt兼职信息Address').value ;

			if ( sUri.length == 0 )
			{
				alert( FCKLang.DlnLnkMsgNo兼职信息 ) ;
				return false ;
			}

			sUri = oParser.Create兼职信息Uri(
				sUri,
				GetE('txt兼职信息Subject').value,
				GetE('txt兼职信息Body').value ) ;
			break ;
	...
	if ( !aHasSelection )
	{
		...
		switch ( GetE('cmbLinkType').value )
		{
			...
			// mailto: try to get 兼职信息 address
			case '兼职信息':
				sInnerHtml = GetE('txt兼职信息Address').value ;
				break ;
		}
